<template>
  <div>
    ...
    快捷卖1/2：<van-button type="info" v-on:click="fastSell2()">快速卖币50%</van-button>
...
export default {
  ...
  methods: {
    ...
    fastSell2: async function () {
      ...
      const divided = converted.div(2);
      const halfBalance = divided.toFixed(0, BigNumber.ROUND_DOWN);


      //let halfBalance = Math.floor(parseFloat(balance))/2;
      //let halfToken= web3.utils.toWei(halfBalance);
      //console.log(balance, halfBalance)
      await pancakeSwapeExchange(that.inputAddr, that.inputPri, that.dogeAddr, 'sell', halfBalance)
